...
function safePlay(sound) { sound.currentTime = 0; sound.play().catch(() => {}); }
...
function updateJoinBtn() {
  // Delegiert an das inline Script in der HTML (hat Zugriff auf currentTab etc.)
...
// Raumcode: uppercase + Teams laden
...
nameInput?.addEventListener("input", () => {});  // Handled by HTML inline script
...
function doJoin() {
  ...
  if (tab === 'create') {
    ...
    if (!teamName) return;
    socket.emit("teams-create-team", { roomCode: rc, name: teamName, colorId: color }, (res) => {
      if (!res?.success) {
        if (joinStatus) joinStatus.textContent = res?.error || "Team konnte nicht erstellt werden";
        return;
      }
      joinAsPlayer(rc, nm, res.teamId);
    });
    // Fallback: server möglicherweise ohne Callback → teams-create-team + player-join-teams separat
    // Daher auch ohne Callback via event:
  } else {
...
function joinAsPlayer(rc, nm, teamId) {
  socket.emit("player-join-teams", { roomCode: rc, name: nm, teamId, hasCamera: false }, (res) => {
    if (res?.success) {
...
function updateBuzzerIndicator() {
  ...
  if (isLocked || !buzzingEnabled) {
    ...
    buzzerBtn.style.borderColor = "rgba(239,68,68,0.5)";
    buzzerBtn.style.color       = "#f87171";
  } else {
...
socket.on("player-buzzed-first", ({ playerId: buzzerId, name, teamId, teamName }) => {
  activeTeamId = teamId;
  renderTeamsBar();
  safePlay(sfxBuzz);
  buzzingEnabled = false;
...
socket.on("spectator-question-opened", ({ categoryIndex, questionIndex, question, value, type, imageUrl }) => {
  ...
  if (type === "image" && imageUrl && qMediaEl && qImageEl) {
    qImageEl.src = imageUrl; qMediaEl.classList.remove("hidden");
...
socket.on("spectator-round-changed", ({ round }) => {
  currentRound = round; usedCells.clear(); buildBoard();
});
