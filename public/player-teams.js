...
// ── Raumcode: uppercase + Teams live laden ──
roomCodeInput?.addEventListener("input", function() {
  this.value = this.value.toUpperCase();
  updateJoinBtn();
  // Teams für diesen Raum anfragen sobald Code eingegeben
  const rc = this.value.trim();
  if (rc.length >= 4) {
    socket.emit("request-teams-local", { roomCode: rc });
  }
});
...
function teamColor(colorId) {
  const map = { red:'#f87171', blue:'#60a5fa', green:'#4ade80', purple:'#c084fc', orange:'#fb923c', pink:'#f472b6' };
...
socket.on("player-buzzed-first", ({ name, teamName }) => {
  if (buzzerStatus) buzzerStatus.textContent = `${name} (${teamName || "Team"}) hat gebuzzert!`;
});
...
// ===============================
// Init — Teams laden sobald Seite lädt
// ===============================
socket.on("connect", () => {
  // Wenn Raumcode schon eingetragen, sofort laden
